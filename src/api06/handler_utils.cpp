...
list<osm_id_t>
parse_id_list_params(FCGX_Request &request, const string &param_name) {
  ...
  if (itr != params.end()) {
    ...
    if (!itr->second.empty()) {
      ...
      try {
        ...
        // note: this is pretty silly, and may change when the rails_port changes
        // to more sensible behaviour... but for the moment we emulate the ruby
        // behaviour; which is that something non-numeric results in a to_i
        // result of zero.
        myids.push_back(0);
      }
    }
