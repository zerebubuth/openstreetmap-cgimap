...
bbox::bbox(double minlat_, double minlon_, 
	   double maxlat_, double maxlon_)
  : minlat(minlat_), minlon(minlon_),
    maxlat(maxlat_), maxlon(maxlon_) {
}

bbox::bbox() 
  : minlat(0.0), minlon(0.0),
    maxlat(0.0), maxlon(0.0) {
}

bool 
bbox::parse(const std::string &s) {
  vector<string> strs;
  ...
  try {
    bbox b(lexical_cast<double>(strs[1]),
	   lexical_cast<double>(strs[0]),
	   lexical_cast<double>(strs[3]),
	   lexical_cast<double>(strs[2]));
