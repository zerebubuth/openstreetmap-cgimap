...
static void get_options(int argc, char **argv, po::variables_map &options) {
  ...
  desc.add_options()
    ...
    ("moderator-ratelimit", po::value<int>(), "average number of bytes/s to allow each moderator")
    ("maxdebt", po::value<int>(), "maximum debt (in Mb) to allow each client before rate limiting")
    ("moderator-maxdebt", po::value<int>(), "maximum debt (in Mb) to allow each moderator before rate limiting")
