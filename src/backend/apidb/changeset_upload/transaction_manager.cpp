...
void Transaction_Manager::prepare(const std::string & name, const std::string & definition)
{
	m_connection.prepare(name, definition);
}

pqxx::prepare::invocation Transaction_Manager::prepared(const std::string &statement)
{
	return m_txn.prepared(statement);
}

pqxx::result Transaction_Manager::exec(const std::string& query, const std::string& description)
{
