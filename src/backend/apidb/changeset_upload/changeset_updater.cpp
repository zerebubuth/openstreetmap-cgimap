...
void ApiDB_Changeset_Updater::lock_current_changeset(bool check_max_elements_limit) {
  ...
  if (check_max_elements_limit && cs_num_changes >= Options::get_instance().get_changeset_max_elements())
    throw http::conflict((boost::format("The changeset %1% was closed at %2%") %
...
{
  {
    ...
    pqxx::result r = m.exec_prepared ("create_changeset", uid, Options::get_instance().get_timeout_idle());
    if (r.affected_rows () != 1)
