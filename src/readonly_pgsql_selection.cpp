...
inline void
insert_results_of(pqxx::work &w, std::stringstream &query, set<id_t> &elems) {
	pqxx::result res = w.exec(query);
...
readonly_pgsql_selection::write_nodes(output_formatter &formatter) {
  ...
  formatter.start_element_type(element_type_node, num_nodes());
	// fetch in chunks...
	set<id_t>::iterator prev_itr = sel_nodes.begin();
	size_t chunk_i = 0;
	for (set<id_t>::iterator n_itr = sel_nodes.begin();
			 ; ++n_itr, ++chunk_i) {
		...
		if ((chunk_i >= STRIDE) || at_end) {
			...
			std::copy(prev_itr, n_itr, infix_ostream_iterator<id_t>(query, ","));
			query << ")";
...
readonly_pgsql_selection::write_ways(output_formatter &formatter) {
  ...
  formatter.start_element_type(element_type_way, num_ways());
	// fetch in chunks...
	set<id_t>::iterator prev_itr = sel_ways.begin();
	size_t chunk_i = 0;
	for (set<id_t>::iterator n_itr = sel_ways.begin();
			 ; ++n_itr, ++chunk_i) {
      ...
      query << "select w.id, w.visible, w.version, w.changeset_id, "
				 ...
				 "current_ways w where w.visible = true and w.id in (";
			std::copy(prev_itr, n_itr, infix_ostream_iterator<id_t>(query, ","));
			query << ")";
...
readonly_pgsql_selection::write_relations(output_formatter &formatter) {
  ...
  formatter.start_element_type(element_type_relation, num_relations());
	// fetch in chunks...
	set<id_t>::iterator prev_itr = sel_relations.begin();
	size_t chunk_i = 0;
	for (set<id_t>::iterator n_itr = sel_relations.begin();
			 ; ++n_itr, ++chunk_i) {
      ...
      query << "select r.id, r.visible, r.version, r.changeset_id, "
				 ...
				 "current_relations r where r.visible = true and r.id in (";
			std::copy(prev_itr, n_itr, infix_ostream_iterator<id_t>(query, ","));
			query << ")";
...
readonly_pgsql_selection::check_node_visibility(id_t id) {
	return check_table_visibility(w, id, "node");
}

data_selection::visibility_t 
readonly_pgsql_selection::check_way_visibility(id_t id) {
	return check_table_visibility(w, id, "way");
}

data_selection::visibility_t 
readonly_pgsql_selection::check_relation_visibility(id_t id) {
...
readonly_pgsql_selection::select_ways_from_nodes() {
  logger::message("Filling sel_ways (from nodes)");
	...
	stringstream query;
	query << "select distinct wn.id from current_way_nodes wn "
		 "where wn.id in (";
...
readonly_pgsql_selection::select_relations_from_ways() {
  logger::message("Filling sel_relations (from ways)");
	...
	stringstream query;
	query << "select distinct id from current_relation_members rm where rm.member_type='Way' "
		 "and rm.member_id in (";
...
readonly_pgsql_selection::select_relations_from_nodes() {
	stringstream query;
	query << "select distinct rm.id from current_relation_members rm "
...
readonly_pgsql_selection::select_relations_from_way_nodes() {
	stringstream query;
	query << "select distinct id from current_relation_members rm "
		 "where rm.member_type='Node' and rm.member_id in (select distinct "
		 "node_id from current_way_nodes where id in (";
	std::copy(sel_ways.begin(), sel_ways.end(), infix_ostream_iterator<id_t>(query, ","));
	query << ")";
	insert_results_of(w, query, sel_relations);
}
