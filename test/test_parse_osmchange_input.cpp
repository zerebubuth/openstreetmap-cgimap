...
void test_way() {
  ...
  // Test node refs up to max number of nodes per way
  {
    ...
    for (int i = 1; i <= Options::get_instance().get_way_max_nodes() + 1; i++) {
      ...
      try {
        process_testmsg(
            (boost::format(
             ...
             os.str())
                .str());
        if (i > Options::get_instance().get_way_max_nodes())
          throw std::runtime_error(
        ...
        const std::string expected = (boost::format(
            ...
            "%3% are allowed") % i % -1 % Options::get_instance().get_way_max_nodes()).str();
        const std::string actual = std::string(e.what()).substr(0, expected.size());
        if (actual != expected)
          throw std::runtime_error(
