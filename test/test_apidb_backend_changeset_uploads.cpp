...
namespace {
  ...
  void test_changeset_update(test_database &tdb) {
      ...
      // Trying to add changeset max elements to empty changeset - should succeed
      try {
	auto upd = tdb.get_data_update();
	auto changeset_updater = upd->get_changeset_updater(1, 1);
	changeset_updater->update_changeset(Options::get_instance().get_changeset_max_elements(), {});  // use undefined bbox
      } catch (http::exception & e) {
	  throw std::runtime_error("test_changeset_update:001 - HTTP Exception unexpected");
      }

      // Trying to add changeset max elements + 1 to empty changeset - should fail
      try {
	auto upd = tdb.get_data_update();
	auto changeset_updater = upd->get_changeset_updater(1, 1);
	changeset_updater->update_changeset(Options::get_instance().get_changeset_max_elements() + 1, {});
      } catch (http::exception &e) {
	  if (e.code() != 409)
