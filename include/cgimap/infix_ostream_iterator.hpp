...
public:
	 infix_ostream_iterator(ostream_type &s) : _M_stream(&s), _M_string(0), _M_first(true) {}
	 infix_ostream_iterator(ostream_type &s, const _CharT *c) : _M_stream(&s), _M_string(c), _M_first(true) {}
	 infix_ostream_iterator(const infix_ostream_iterator &obj) : _M_stream(obj._M_stream), _M_string(obj._M_string), _M_first(obj._M_first) {}
	
	 infix_ostream_iterator &operator=(const _Tp &value) {
		 __glibcxx_requires_cond(_M_stream != 0, _M_message(__gnu_debug::__msg_output_ostream)._M_iterator(*this));
